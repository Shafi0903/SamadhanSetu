...
* **[2026-09-22] Production Security Hardening Patch:** Integrated `helmet` HTTP defense headers (`nosniff`, `x-frame-options: SAMEORIGIN`, `x-download-options`), strict dynamic CORS origin policy allowing only trusted Vercel and local development origins, and `express-rate-limit` middlewares (`authRateLimiter` 30 attempts/15min on auth endpoints, `apiRateLimiter` 500 requests/15min globally) with reverse proxy trust.
...
* **[2026-09-22] Comprehensive Pilot Database Seed Script:** Built realistic seeding pipeline (`apps/backend/prisma/seed.ts`) creating test citizen accounts, KMC (Kolkata Municipal Corporation) authorities, Jadavpur University and IIEST Shibpur research faculties, Tata Trusts and Infosys Foundation CSR partners, geotagged civic challenges across Kolkata (Park Street, Gariahat, Koley Market, Maa Flyover), solution prototypes, and timeline histories.
